...
> 빠른 구분: **context7 MCP**는 Node.js만 있으면 `npx`로 바로 실행되는 문서 조회 서버다. **Godot MCP**는 에디터 제어용 선택 서버라 Coding-Solo/godot-mcp 빌드와 Godot 실행 파일 경로 설정 전에는 실패하거나 비활성일 수 있다. Godot MCP가 실패해도 context7과 일반 코드/문서 작업은 별개로 동작한다.

---

## GitHub 이슈 라벨 준비

새 레포로 사용할 때는 설정 이슈 템플릿(`.github/ISSUE_TEMPLATE/setup_config.md`)이 사용하는
`config` 라벨을 미리 준비한다. 이 라벨은 설정·툴링·프로젝트 세팅 변경 이슈를 분류하는 용도다.

```bash
gh label create config \
  --repo claude-studio/godot-template \
  --description "Configuration, tooling, and project setup changes" \
  --color "5319e7"
```

> 위 명령은 예시다. 이 문서는 라벨 준비 방법만 안내하며, 라벨을 자동으로 생성하지 않는다.
...
`.mcp.json`의 godot 서버 선언은 환경변수 두 개를 그대로 사용한다(참조용). Claude Code용 JSON은 주석을 지원하지 않으므로 서버 선언은 남겨 두고, 환경변수로 실제 경로를 주입한다:
...
> 메모: `.mcp.json`은 `${GODOT_MCP_PATH:-…}`·`${GODOT_PATH:-}` 형태의 **기본값**을 써서, 변수가 미설정이어도 설정 파싱이 깨지지 않는다. 따라서 두 변수가 미설정/오경로면 **godot 서버만 실패/비활성**될 뿐 context7은 정상 동작한다(전체 MCP 설정이 함께 무력화되지 않음). `/mcp`에서 `context7`은 정상이고 `godot`만 실패한다면 Godot MCP 경로 설정만 다시 확인하면 된다.
...
이 리포에는 Codex용 `.codex/config.toml`이 포함돼 있다. 새 클론에서 placeholder 경로로 Godot MCP 시작 실패가 나지 않도록 기본 활성 서버는 `context7`만 두고, `godot` 서버는 주석 예시로 제공한다.
...
- `godot` 서버는 **절대경로 수정 후 주석 해제**가 필요하다. `.codex/config.toml`을 열어 주석 처리된 예시를 복사/해제한 뒤 다음을 환경에 맞게 바꾼다(TOML은 환경변수를 자동 확장하지 않음):
...
- `context7`만 필요하다면: `node --version` / `npx --version`이 정상인지 확인한다. Godot MCP 경로는 설정하지 않아도 된다.
- Claude Code에서 `godot` MCP가 실패한다면: `echo "$GODOT_MCP_PATH"`, `ls "$GODOT_MCP_PATH/build/index.js"`, `echo "$GODOT_PATH"`를 확인한다.
- Codex에서 `godot` MCP가 필요하다면: `.codex/config.toml`의 주석 예시를 실제 절대경로로 바꿔 주석 해제했는지 확인한다. `/ABSOLUTE/PATH/TO/...` 같은 placeholder는 남기지 않는다.
- `godot` MCP만 실패하고 `context7`은 뜬다면 전체 MCP 문제가 아니라 에디터 제어 선택 기능의 경로 문제다.

### 8-4. 스킬·서브에이전트는 어떻게 공유되나
