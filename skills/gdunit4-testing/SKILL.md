...
> Godot 4.4.x에서는 GdUnit4 `v4.4.3`을 확인했으며, 수동 배치 시 `addons/gdUnit4/test`는 제외한다(애드온 자체 fixture의 `Player` 이름이 템플릿 `Player` 전역 클래스와 충돌할 수 있음).
...
```bash
# 전역 클래스/애드온 캐시를 먼저 생성
# (최초 클린 클론이나 애드온을 새로 복사한 직후 필요)
