...
GdUnit4는 Godot 애드온이며 **설치가 필요**하다. 실행 전에 `addons/gdUnit4` 디렉터리가 있는지 먼저 확인한다. 클린 클론에는 애드온이 포함되어 있지 않으므로, 디렉터리가 없으면 테스트 실패가 아니라 “GdUnit4 미설치로 unit test 미실행”으로 분류한다.

```bash
ls addons/gdUnit4
```

없으면 테스트가 동작하지 않는다. `docs/SETUP.md`의 GdUnit4 설치 절차(Godot AssetLib 또는 `git clone https://github.com/godot-gdunit-labs/gdUnit4` 후 `addons/gdUnit4` 배치)를 안내하고 멈춘다. 임의로 설치하지 않는다. 코드/씬 변경의 기본 smoke 검증은 별도로 `godot --headless --path . --quit-after 10`을 실행해 parse/import/runtime 시작 오류를 확인한다.
...
- macOS/Linux:
  ...
  Godot 4.4.x에서는 GdUnit4 `v4.4.3` + `addons/gdUnit4/test` 제외 배치를 확인했다. 애드온 자체 테스트 fixture를 함께 두면 프로젝트의 `Player` 전역 클래스와 fixture 이름이 충돌할 수 있다.
...
- 러너가 Godot 실행 파일 경로를 요구하면 `GODOT_BIN` 환경변수로 지정한다(예: macOS `export GODOT_BIN=/Applications/Godot.app/Contents/MacOS/Godot`). CI/헤드리스 환경에서는 Godot가 `--headless`로 구동되어 GPU 없이도 동작한다. macOS saved-state 모달로 CLI가 멈추면 `-ApplePersistenceIgnoreState YES`를 넣은 래퍼 스크립트를 만들고 그 경로를 `GODOT_BIN`으로 지정한다.
