...
- **특정 씬 실행** — `$ARGUMENTS`가 있을 때. 공식 표기: positional 인자는 **프로젝트 상대경로**로, `res://`·UID 경로는 **`--scene`** 플래그로 넘긴다.

  ```bash
  # 상대경로(권장)
  godot --path . scenes/main.tscn
  # res:// 또는 UID 경로는 --scene 사용
  godot --path . --scene res://scenes/main.tscn
  ```

  예: `godot --path . src/entities/player/player.tscn` 또는 `godot --path . --scene res://src/entities/player/player.tscn`. (`$ARGUMENTS`가 `res://` 접두면 `--scene`으로, 상대경로면 positional로 넘긴다.)
